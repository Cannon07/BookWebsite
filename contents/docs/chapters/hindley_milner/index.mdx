...
</div> <div className="code-block-with-latex">
<pre><code className="language-python">
...
</div> The `unify_variable` algorithm solves
...
</div> The helper method `occurs` checks
...
contained in the subtree. <div className="code-block-with-latex">
<pre><code className="language-python">
[language=python]
...
</div> Here we use a helper method
...
1.  Use the typing rules to determine the type of the following
    expression. <div className="code-block-with-latex">
<pre><code className="language-python">
let x = 5 in
    $\lambda$ y . x + y
</code></pre>
