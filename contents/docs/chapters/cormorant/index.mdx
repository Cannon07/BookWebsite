...
$$\begin\{aligned\}
_m &= \sqrt\{\frac\{4\pi\}\{2 \ell + 1\} \} \sum_\{i=1\}^N q_i (r_i)^\ell Y^m_\ell(\theta_i, \phi_i)
\end\{aligned\}$$
...
</div> where $d$ is the dimensionality of the
...
</div> Here $Z_i$ is the atom type and $r_i$
...
</div> The `CGLayer` is built out of three
