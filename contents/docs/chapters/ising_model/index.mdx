...
![Schematic representation of different 2D lattice
structures](/BookWebsite/figures/ising_model/lattice.png)
...
(Fig. [$$lattice$$]\{reference-type="ref"
...
![Ground states (minimum energy) for the 2-dimensional Ising model
(square lattice) for a ferromagnetic system (with coupling constant
$J >0$) and an antiferromagnetic system (with coupling constant
$J < 0$).](/BookWebsite/figures/ising_model/ferromagnetic_ground_states.png)
...
(Fig. [$$fig1$$]\{reference-type="ref" reference="fig1"\}):
...
![Spontaneous
Magnetization](/BookWebsite/figures/ising_model/spontaneous_magnetization.png)
...
Temperature (Fig.[$$fig2a$$]\{reference-type="ref"
reference="fig2a"\})
...
![The total magnetization $M=N$ versus the external field $\beta h$ for
the non-interacting Ising model
$J = 0$).](/BookWebsite/figures/ising_model/total_magnetization.png)

The non-interacting Ising model ($J =0$) exhibits a gradual change in
the magnetization $M$ with the external field $h$
(Fig. [$$fig2$$]\{reference-type="ref" reference="fig2"\}). Thus,
...
where $Tr\{T^N\}$ indicates a trace of the matrix $T^N$.
...
![image](/BookWebsite/figures/ising_model/phase_transition_1d.png)
![Energy cost of a phase transition from an ordered phase to a
disordered phase in 1D and
2D.](/BookWebsite/figures/ising_model/phase_transition_2d.png)
...
(Fig. [$$fig4$$]\{reference-type="ref" reference="fig4"\}). In the
next lecture, we test the limits of this approximation.

![Schematic of the mean-field
approximation.](/BookWebsite/figures/ising_model/mean_field_approx.png)
...
![Plots of $\tanh (Km)$ versus $m$ for various values of $K$. Circles
mark points that satisfy the self-consistent equation
$m = \tanh (Km)$.](/BookWebsite/figures/ising_model/tanh_K_plot.png)
$$\begin\{aligned\}
\label\{Eq8\}
...
(Fig. [$$fig5$$]\{reference-type="ref" reference="fig5"\}):

![Phase diagram for the mean-field solution for the Ising model,
showing $m$ versus
$T/T_c$.](/BookWebsite/figures/ising_model/ising_phase_diagram.png)
...
(Fig. [$$fig6$$]\{reference-type="ref" reference="fig6"\}).
...
![Plot of $m$ versus $\beta h$ for several values of
$K = \beta Jz = T_c=T$. The dashed part of each curve indicates where
the system is
unstable.](/BookWebsite/figures/ising_model/ising_plot_m_bh.png)

The phase diagram for the mean-field solution is dictated by the
self-consistent mean field equation
(Fig. [$$fig7$$]\{reference-type="ref" reference="fig7"\}):
$$\begin\{aligned\}
\label\{Eq12\}
...
variety of phenomena (Fig. [$$fig9$$]\{reference-type="ref"
reference="fig9"\}).

![Plot of $m$ versus $\beta h$ for $K = \beta Jz = T_c/T = 1.5$. The
arrows indicate the hysteresis in
magnetization.](/BookWebsite/figures/ising_model/ising_hysteresis.png)
