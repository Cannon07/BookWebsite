...
Below we show an example for the Allen-Cahn equation that appears in systems with phase separations, e.g., battery electrode-electrolyte interfaces. \\
\begin{example}[Allen-Cahn Equations]
 The example is used to demonstrate the ability of the proposed discrete time models to handle different types of nonlinearity in the governing partial differential equation. Let's consider the Allen–Cahn equation along with periodic boundary conditions
...
%A summary of The results for this example is presented in the following figures.
...
Universal differential equations (UDEs) are an approach can be utilized to discover previously unknown governing equations, accurately extrapolate beyond the original data, and accelerate model simulation, all in a time and data-efficient manner.



\section{PINNs}

\paragraph{Broad Goal:}\label{broad-goal}
...
\item
  ...
  \itemsep1pt\parskip0pt\parsep0pt
  \item
    Leverage NNs and their well known capability as universal function
    approximators
  \item
...
\paragraph{Two Distinct Algorithms:}\label{two-distinct-algorithms}
...
\section{Data-driven Solutions of Partial Differential
Equations}\label{data-driven-solutions-of-partial-differential-equations}
...
\paragraph{Specifically:}\label{specifically}

\begin{itemize}
\itemsep1pt\parskip0pt\parsep0pt
\item
  \textbf{Given fixed model parameters $\lambda$, what can be said about the
  unknown hidden state u(t, x) of the system}
\item
  \textbf{Data driven discovery of partial differential equations: what
  are the parameters $\lambda$ that best describe the observed data?}
\end{itemize}

\section{Continuous Time Models: General
Formalism}\label{continuous-time-models-general-formalism}
...
in a \textbf{physics informed neural network}, $f(t,x)$.
...
Parameters}\label{learning-the-neural-network-parameters}
...
\subsection{Example: Burgers' Equation}\label{example-burgers-equation}

Consider the \textbf{Burgers' equation}. In one space dimension, the
Burger's equation along with Dirichlet boundary conditions reads as
...
(contd.)}\label{example-burgers-equation-contd.}
...
\subsection{Example: Burgers' Equation
(contd.)}\label{example-burgers-equation-contd.-1}
...
\textgreater{} \emph{Burgers' equation:} \emph{Top:} Predicted solution
along with the initial and boundary training data. In addition we are
using 10,000 collocation points generated using a Latin Hypercube
Sampling strategy. \emph{Bottom:} Comparison of the predicted and exact
solutions corresponding to the three temporal snapshots depicted by the
white vertical lines in the top panel. Model training took approximately
60 seconds on a single NVIDIA Titan X GPU card.
...
\paragraph{Example: Navier-Stokes Equation
(Results)}\label{example-navier-stokes-equation-results}
\begin{figure}
    \centering
...
\paragraph{Example: Navier-Stokes Equation
(Results)}\label{example-navier-stokes-equation-results-1}
