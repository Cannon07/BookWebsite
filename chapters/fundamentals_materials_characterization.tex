...
\chapter{Fundamentals of Material Characterization}

\label{chap:fund_matchar}
...
Modern ML methods use a variety of input data from materials characterization -- on which build ML models -- associated output data.

In this overview chapter, we review different techniques for characterizing the properties of materials with the aim of giving you a high level overview of different techniques for characterizing materials properties.
 
\section{Microscopy}

Microscopy is a category of material characterization techniques which probe and map the surface and sub-surface structure of a material. These techniques can use photons, electrons, ions or physical cantilever probes to gather data about a sample's structure on a range of length scales. Some common examples of microscopy instruments include:
\begin{itemize}
\item Optical Microscope
\item Scanning Electron Microscope (SEM)
\item Transmission Electron Microscope (TEM)
\item Field Ion Microscope (FIM)
\item Scanning Tunneling Microscope (STM)
\item Scanning probe microscopy (SPM)
\item Atomic Force Microscope (AFM)
\item X-ray diffraction topography (XRT)
\end{itemize}
...
Microscopy methods provide powerful tools to inspect the local structure of materials. Microscopy can be performed at human-visible wavelengths, or alternatively with UV light.

%\subsection{Optical radiation}
%\textit{Ultraviolet-visible spectroscopy (UV-vis)}\\
%\textit{Fourier transform infrared spectroscopy (FTIR)}\\
\subsection{X-Rays}
...
%\textit{Electron energy loss spectroscopy (EELS)}\\
%\textit{X-ray photoelectron spectroscopy (XPS)}\\
%\subsection{Nuclear spectroscopy}
%\textit{Nuclear magnetic resonance spectroscopy (NMR)}\\
%\textit{Mössbauer spectroscopy (MBS)}\\


%\section{EM Waves and Photons (Neutrons maybe)}
\section{XPS}
X-ray photoelectron spectroscopy (XPS) is a surface-sensitive quantitative spectroscopic technique based on the photoelectric effect that can identify the elements that exist within a material (elemental composition) or are covering its surface, their chemical state, as well as the overall electronic structure and density of the electronic states in the material. XPS is a powerful measurement technique because it not only shows what elements are present, but also what other elements they are bonded to. The technique can be used in line profiling of the elemental composition across the surface, or in depth profiling when paired with ion-beam etching. It is often applied to study chemical processes in the materials in their as-received state or after cleavage, scraping, exposure to heat, reactive gasses or solutions, ultraviolet light, or during ion beam implantation.
\section{Raman}
