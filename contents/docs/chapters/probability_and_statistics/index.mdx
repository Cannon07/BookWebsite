...
the previous. If we track the evolution of the outcomes, we would
observe that no conclusions can be drawn from the first few samples, but
rather after performing a big number of experiment, where we would be
able to see a clear tendency of $1/2$.
...
Using and, we have $N_H=(N+X)/2$ and $N_T=(N-X)/2$. This gives:
...
probability of getting $N_1, N_2, . . . , N_r$ is:
$$\frac\{N!\}\{N_1!N_2!...N_r!\}P_1^\{N_1\}P_2^\{N_2\}...P_r^\{N_r\}$$

which is a term in the multinomial expansion:
...
probabilities resemble a continuous probability distribution. We call
...
![Curves of the Poisson distribution versus $n$ for $a =$ 0, 2, 4, 6,
8, and
10.](/BookWebsite/figures/probability_and_statistics/poisson.png)
