...
\
**Prerequisites:**
[$$chap:math_basics$$]\{reference-type="ref+label"
...
![Distinction between bracketing methods (left) and open methods
(right)](/figures/newton_raphson/openmethod.PNG) In the bracketing
methods of root-finding, the root is located within an interval
prescribed by a lower and an upper bound. In contrast, open methods
require only a single starting value or two starting values that do not
necessarily bracket the root. In *bracketing methods*, the estimate of
the root gets better with every iteration. This cannot be ensured in the
case of *open methods*. However, when the open methods converge, they
converge much faster than the bracketing methods. The distinction
between bracketing methods and open methods is shown in Figure
[$$fig:1$$]\{reference-type="ref" reference="fig:1"\} by depicting
Bisection (bracketing method) and Newton-Raphson (open method).

## Newton-Raphson Method

![Graphical depiction of Newton Raphson
method](/figures/newton_raphson/newtonraphson.PNG)
...
be observed from Figure [$$fig:2$$]\{reference-type="ref"
...
[$$fig:3$$]\{reference-type="ref" reference="fig:3"\}. In (a), if
...
![Cases where Newton-Raphson demonstrates poor
convergence](/figures/newton_raphson/newtonraphsonpoor.PNG)
...
![image](/code-blocks/newton_raphson/code_block_0_37a5797d.png) Next we implement Newton-Raphson
![image](/code-blocks/newton_raphson/code_block_1_84b90bbd.png)
...
![image](/code-blocks/newton_raphson/code_block_2_da94032e.png)
...
![image](/code-blocks/newton_raphson/code_block_3_3ad0e744.png)
