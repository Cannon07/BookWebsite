...
![Graphical depiction of the Simpson's Rule. (a) Area under a parabola.
(b) Area under a
cubic](/BookWebsite/figures/simpsons_rule/polynomials.PNG) To obtain a
more accurate estimate of an integral (instead of finer segmentation of
the trapezoidal rule) is to use higher-order polynomials to connect the
points. For example, if there is an extra point midway between $f(a)$
and $f(b)$, the three points can be connected with a parabola (Fig
[$$fig:1$$]\{reference-type="ref" reference="fig:1"\}a). If there
are two points equally spaced between $f(a)$ and $f(b)$, the four points
can be connected with a third-order polynomial (Fig
[$$fig:1$$]\{reference-type="ref" reference="fig:1"\}b). The
formulas that result from taking the integrals under these polynomials
are called Simpson's rules. These formulas take inspiration from the
Taylor series expansion, as in approximating the function with a higher
order polynomial (rather than a linear function) for more accurate
representation.
...
![image](/BookWebsite/code-blocks/simpsons_rule/code_block_0_a99cf208.png)
...
![Graphical representation of the relative weights used in the
Composite Simpson's 1/3
rule](/BookWebsite/figures/simpsons_rule/composite.png)
...
![image](/BookWebsite/code-blocks/simpsons_rule/code_block_2_f7f3e100.png)
...
![Applying Simpson's 1/3 and 3/8 rules in tandem to handle odd number
of intervals](/BookWebsite/figures/simpsons_rule/simpsonsodd.png)
...
[$$fig:3$$]\{reference-type="ref" reference="fig:3"\}. In this
