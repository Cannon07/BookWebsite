...
2017a]; [Lazer et al. 2014]). In this chapter, we
...
papers in hydrology ([Gupta et al. 2014]; [Lall 2014];
...
to physics-based models ([Goh] [et al. 2017];
[Graham-Rowe et al. 2008]). However, direct application of
...
2017]; [San and] [Maulik 2018]; [Wan et al.
2018]). One of the key limitations of these approaches is their
inability to provide predictions that are consistent with known physical
laws. Karpatne et al. ([Karpatne et al. 2017b]) further extends
...
([Beucler et al. 2019]; [Karpatne et al. 2017b];
[Muralidhar] [et al. 2018]). However, these methods
...
2.  The proposed PGRNN can incorporate explicit physical laws such as
    ...
    include a physics-based penalty ([Karpatne et al. 2017a]).
...
1979]) that controls the growth, survival, and reproduction of
fish ([Roberts] [et al. 2013]). Warming water
temperatures can increase the occurrence of aquatic invasive species
([Rahel and Olden][2008]; [Roberts et al.
2017]), which may displace fish and native aquatic organisms,
result in more harmful algal blooms (HABs) ([Harris and Graham
2017]; [Paerl and Huisman 2008]). Understanding
...
([Hipsey et al. 2019]), which simulates the physical processes
...
2020]; [Read et al. 2019]). As discussed in
([Willard] [et al. 2020]), the overall approach is
...
2019]). Given these input drivers x~t~ and a depth level d, we
...
(shown in Fig. [1]). We use GLM as the preferred physics-based
...
energy constraints (see Section [3.2]). Since we train machine
...
1994]). Therefore, the RNN-based method may fail to grasp
...
1992]; [Pan and] [Duraisamy 2018]) that such
...
(Eqs. [2]-[5]) can be applied to all the time steps.
...
any energy losses from the lake (see Fig. [3]). The explicit
...
In Fig. [2], we show the fow of the proposed PGRNN model, which
...
periods. However, Eq. [8] should always hold for data from any
...
Eqs. [11]-[13], the heat fuxes and lake energy are
...
2017]). Given their complexity, the RNN-based models trained
