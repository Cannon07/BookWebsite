...
\chapter{Numerical Integration with Gaussian Quadrature}
\label{chap:gauss_quadrature}
\noindent\rule{11cm}{0.4pt} \\
\textbf{Prerequisites:} \autoref{chap:calculus}, \autoref{chap:trapezoidal} \\
\textbf{Difficulty Level:} ** \\
\noindent\rule{11cm}{0.4pt}
\vspace{5mm}

In this chapter, we learn how Gaussian quadrature provides for superior integral estimates by picking optimal abcissas at which to evaluate the function. We study the importance of Gauss-Hermite and Gauss-Laguerre quadratures for estimating integrals with $\infty$ limits. We then study the use of Gauss-Legendre quadrature for approximating integrals with finite limits.

\section{Gaussian Quadrature}

So far, we have employed the Newton-Cotes equations to evaluate the integrals. However, these methods provide estimates for integrals but are not exact. In addition, they cannot be used to evaluate improper integrals, which are definite integrals that have either or both of the limits infinite. These cases will have to be approximated further and evaluated using large numbers tending to infinity. For example, consider the integral $\int_{-\infty}^{\infty}e^{-x^2}\tan(x)dx$. The methods discussed in the previous chapters (Trapezoidal and Simpson's Rule) will yield results with large error. The Gauss quadrature methods discussed in this chapter will enable us to evaluate these integrals either exactly or with high accuracy.

In numerical analysis, \textit{quadrature} rule is an approximation of the integral of a function, usually stated as a weighted sum of function values at specified points within the domain of integration. By choosing these weights and positioning these specified points mathematically wisely, a very accurate estimate of the integral is obtained. These quadrature methods extend to integrals with infinite limits in addition to cases with finite limits. Note that any integral with finite limits can be converted to an integral going from $-1$ to $1$ by a simple change of variables, which will be used in the Gauss-Legendre quadrature later. The general $n$-point quadrature rule is given as
\begin{align}
...
%So far, we have employed the Newton-Cotes equations to evaluate the integrals. However, these methods provide estimates for integrals but are not exact. In addition, they cannot be used to evaluate improper integrals, which are definite integrals that have either or both of the limits infinite. These cases will have to be approximated further and evaluated using large numbers tending to infinity. For example, consider the integral $\int_{-\infty}^{\infty}e^{-x^2}\tan(x)dx$. The methods discussed in the previous lectures (Trapezoidal and Simpson's Rule) will yield results with large error. The Gauss quadrature methods discussed in this lecture will enable us to evaluate these integrals either exactly or with high accuracy.
...
\noindent where the $c$’s are the unknown coefficients. Thus, we now have a total of four unknowns $c_0, c_1, x_0, x_1$ that must be evaluated, and consequently, we require four conditions to determine them exactly.
...
\begin{marginfigure}[\baselineskip] 
	\centering
	% \includegraphics[width = 2.7in]{figures/part1b/gauss_quadrature/twopoint.PNG}
 \includegraphics[width = 2.7in]{figures/part1b/gauss_quadrature/twopoint01.png}
	\centering
	\caption{Graphical depiction of the unknown variables $x_0$ and $x_1$ for integration by Gauss quadrature}
	\label{fig:3}
\end{marginfigure}
...
Let's try
evaluating $ \int_{-1}^{1}(5x^3 + 7x^2 +  2x + 3)dx$
...
\end{align} 
where the $c$’s are the unknown coefficients. Thus, we again have a total of four unknowns $c_0, c_1, x_0, x_1$ that must be evaluated, and consequently, we require four conditions to determine them exactly.
...
evaluating $ \int_{0}^{\infty}\log(1+e^{-x})dx$
