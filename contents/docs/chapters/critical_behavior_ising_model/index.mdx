...
In [$$chap:ising_model$$]\{reference-type="ref+label"
...
(Fig. [$$p-V$$]\{reference-type="ref" reference="p-V"\}). As the
system approaches the critical point, the density fluctuations dominate
the physical behavior.

![Plot of p versus v for a = 3.375 = 27/8 (critical point, blue), a =
3.527, a = 3.679, a = 3.831, and a = 3.982 (red). The solid black curves
are the binodal curves (coexistence), and the dashed black curves are
the spinodal curves (limit of metastability). The critical point is
identified as the point where the spinodal curves
converge.](/BookWebsite/figures/critical_behavior_ising_model/fig1.png)
...
(Fig. [$$rayleigh$$]\{reference-type="ref"
reference="rayleigh"\}).
...
\end\{aligned\}$$ ![The sky is blue due to a phenomenon called Rayleigh
scat- tering, where light is scattered due to local density fluctuations
in the fluid. Such density fluctuations occur for a fluid that is near
its critical point, which is the case for the oxygen and nitrogen in the
upper atmosphere. The shortest wavelength light (violet end of spec-
trum) is scattered the most, leading to an overall perceived color that
is light
blue.](/BookWebsite/figures/critical_behavior_ising_model/fig2.jpg)
...
![Snapshots from a Monte Carlo simulation of the 2-D Ising model on a
square lattice (32x32) for various temperatures $T$ . The Monte Carlo
simulation exhibits an order-disorder phase transition between $T = 2.3$
and $T = 2.2$; note, the 2D phase transition occurs at
$T = 2.269$.](/BookWebsite/figures/critical_behavior_ising_model/fig3.png)
...
Fig. [$$mc$$]\{reference-type="ref" reference="mc"\}.
