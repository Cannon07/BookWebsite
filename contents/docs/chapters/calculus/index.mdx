...
![A plot of
$f(x) = |x|$](/BookWebsite/figures/calculus/absolute_value_x.png)
...
$\mathbb\{R\} \to \mathbb\{R\}$ such that $$\frac\{d\}\{dx\} \int f dx = f
    $$ Antiderivatives are not unique. The reason for
...
3.  Prove that the antiderivative of $f(x) = e^x$ is $e^x + C$ by using
    the definiting Equation. Hint: Consider the Taylor series.
